    ...
    try:
        ...
        try:
            ...
            if catalogs:
                ...
                console.print(
                    f"✅ Catálogos disponíveis: [bold]{names}[/bold]"
                    + (f" (+ {len(catalogs) - 5} more)" if len(catalogs) > 5 else "")
                )
            else:
                console.print(
                    "⚠️  Nenhum catálogo encontrado — Unity Catalog pode não estar habilitado."
                )
        ...
        console.print(
            "Por favor, verifique suas variáveis de ambiente: DATABRICKS_HOST e DATABRICKS_TOKEN."
        )
