...
class LivyClient:
    ...
    async def _get_or_create_session(self, workspace_id: str, lakehouse_id: str) -> int:
    ...
    async def execute_sql(
        ...
        url = f"{self._livy_base(workspace_id, lakehouse_id)}/sessions/{session_id}/statements"
                ...
                if state == "available":
                    ...
                    if output.get("status") == "ok":
                        ...
                        if "application/json" in result:
                            json_data = result["application/json"]
                            return {
                                "columns": json_data.get("schema", {}).get("fields", []),
                                "rows": json_data.get("data", []),
                            }
                        return {"columns": [], "rows": []}
                    else:
                        raise RuntimeError(f"Spark SQL error: {output.get('evalue', 'Unknown')}")
                elif state in ("error", "cancelled"):
