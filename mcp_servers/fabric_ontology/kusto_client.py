...
class KustoClient:
    ...
    async def list_tables(self, cluster_uri: str, database: str) -> list[str]:
        """List all table names in a KQL database."""
        frames = await self.execute_query(cluster_uri, database, ".show tables | project TableName")
        tables: list[str] = []
        for frame in frames:
    ...
    async def get_table_schema(
        ...
        """
        frames = await self.execute_query(cluster_uri, database, f"{table_name} | getschema")
