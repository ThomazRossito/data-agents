...
def encode_definition(decoded: dict[str, Any]) -> list[dict]:
    ...
    for et_id, et_data in decoded.get("entityTypes", {}).items():
        ...
        if et_data.get("overviews"):
            parts.append(
                _make_part(f"EntityTypes/{et_id}/Overviews/definition.json", et_data["overviews"])
            )

        if et_data.get("resourceLinks"):
            parts.append(
                _make_part(
                    f"EntityTypes/{et_id}/ResourceLinks/definition.json", et_data["resourceLinks"]
                )
            )
    ...
    for rt_id, rt_data in decoded.get("relationshipTypes", {}).items():
        if rt_data.get("definition"):
            parts.append(
                _make_part(f"RelationshipTypes/{rt_id}/definition.json", rt_data["definition"])
            )

        for ctx in rt_data.get("contextualizations", []):
            ctx_id = ctx.get("id", "unknown")
            parts.append(
                _make_part(f"RelationshipTypes/{rt_id}/Contextualizations/{ctx_id}.json", ctx)
            )
