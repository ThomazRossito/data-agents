...
class OneLakeClient:
    ...
    async def list_schemas(self, workspace_id: str, item_id: str) -> list[dict[str, Any]]:
    ...
    async def get_table(
        ...
        resp = await client.get(url, headers=headers)
        if resp.status_code != 200:
            raise RuntimeError(f"OneLake get_table failed ({resp.status_code}): {resp.text[:500]}")
        return resp.json()
